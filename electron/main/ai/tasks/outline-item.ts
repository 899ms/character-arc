...
export function normalizeOutline(parsed: Partial<OutlineResult>): OutlineResult {
  return {
    ...
    wordTarget: parsed.wordTarget?.trim() || '预估 3000字',
    conflict: parsed.conflict?.trim() || '新的冲突正在酝酿。',
    summary: parsed.summary?.trim() || 'AI 未返回有效剧情摘要'
  }
}
...
const handler: TaskHandler = {
  name: 'outline-item',
  outputType: 'json',
  defaultCapabilities: ['settings', 'outline', 'worldview', 'characters', 'writing-style', 'project-skills'],
